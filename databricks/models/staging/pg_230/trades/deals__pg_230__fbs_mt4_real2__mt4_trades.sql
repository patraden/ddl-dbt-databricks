{{ config(
        pre_hook="CREATE TABLE if not exists {{ source('dw', 'deals__pg_230__fbs_mt4_real2__mt4_trades') }} USING parquet LOCATION 'gs://staging-databricks-{{ var('env') }}/pg_230__deals/fbs_mt4_real2__mt4_trades/'",
    ) }}
with src as
(
select
...
from {{ source('dw', 'deals__pg_230__fbs_mt4_real2__mt4_trades') }}
)
select
