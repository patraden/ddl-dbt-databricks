{{ config(
        pre_hook="CREATE TABLE if not exists {{ source('dw', 'deals__pg_230__asic_mt4_demo1__mt4_trades') }} USING parquet LOCATION 'gs://staging-databricks-{{ var('env') }}/pg_230__deals/asic_mt4_demo1__mt4_trades/'",
    ) }}
with src as
(
select
