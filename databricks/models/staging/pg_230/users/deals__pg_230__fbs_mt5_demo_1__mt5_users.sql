{{ config(
        pre_hook="CREATE TABLE if not exists {{ source('dw', 'deals__pg_230__fbs_mt5_demo_1__mt5_users') }} USING parquet LOCATION 'gs://staging-databricks-{{ var('env') }}/pg_230__deals/fbs_mt5_demo_1__mt5_users/'",
    ) }}
with src as
(
select
...
from {{ source('dw', 'deals__pg_230__fbs_mt5_demo_1__mt5_users') }}
)
select
