{{ config(
        pre_hook="CREATE TABLE if not exists {{ source('dw', 'deals__pg_230__fbs_tp_real_1__mt5_deals') }} USING parquet LOCATION 'gs://staging-databricks-{{ var('env') }}/pg_230__deals/fbs_tp_real_1__mt5_deals/'",
    ) }}
with src as
(
select
...
from {{ source('dw', 'deals__pg_230__fbs_tp_real_1__mt5_deals') }}
)
select
